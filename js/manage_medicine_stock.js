function deleteMedicineStock(id) {
  ...
  if (confirmation) {
    ...
    xhttp.open(
      "GET",
      "includes/manage_medicine_stock.php?action=delete&id=" + id,
      true
    );
    xhttp.send();
  }
...
function editMedicineStock(id) {
  ...
  xhttp.open(
    "GET",
    "includes/manage_medicine_stock.php?action=edit&id=" + id,
    true
  );
  xhttp.send();
}
...
function updateMedicineStock(id) {
  ...
  if (!notNull(batch_id.value, "batch_id_error")) batch_id.focus();
  else if (!checkExpiry(expiry_date.value, "expiry_date_error"))
    expiry_date.focus();
  else if (!checkQuantity(quantity.value, "quantity_error")) quantity.focus();
  else if (!checkValue(mrp.value, "mrp_error")) mrp.focus();
  else if (!checkValue(rate.value, "rate_error")) rate.focus();
    ...
    xhttp.open(
      "GET",
      "includes/manage_medicine_stock.php?action=update&id=" +
        id +
        "&batch_id=" +
        batch_id.value +
        "&expiry_date=" +
...
function cancel() {
  ...
  xhttp.open("GET", "includes/manage_medicine_stock.php?action=cancel", true);
  xhttp.send();
}
...
function searchMedicineStock(text, tag) {
  ...
  xhttp.open(
    "GET",
    "includes/manage_medicine_stock.php?action=search&text=" +
      text +
      "&tag=" +
      tag,
    true
