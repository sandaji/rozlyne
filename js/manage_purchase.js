function deletePurchase(id) {
  ...
  if (confirmation) {
    ...
    xhttp.open(
      "GET",
      "includes/manage_purchase.php?action=delete&id=" + id,
      true
    );
    xhttp.send();
  }
...
function editPurchase(id) {
  ...
  xhttp.open("GET", "includes/manage_purchase.php?action=edit&id=" + id, true);
  xhttp.send();
}
...
function updatePurchase(id) {
  ...
  //}
  //else
  if (!checkDate(invoice_date.value, "date_error")) invoice_date.focus();
  else {
    var xhttp = new XMLHttpRequest();
    xhttp.onreadystatechange = function () {
      if ((xhttp.readyState = 4 && xhttp.status == 200))
    ...
    xhttp.open(
      "GET",
      "includes/manage_purchase.php?action=update&id=" +
        id +
        "&suppliers_name=" +
...
function cancel() {
  ...
  xhttp.open("GET", "includes/manage_purchase.php?action=cancel", true);
  xhttp.send();
}
...
function searchPurchase(text, tag) {
  ...
  xhttp.open(
    "GET",
    "includes/manage_purchase.php?action=search&text=" + text + "&tag=" + tag,
    true
  );
  xhttp.send();
}
