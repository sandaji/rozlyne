function deleteInvoice(invoice_number) {
  ...
  if (confirmation) {
    ...
    xhttp.open(
      "GET",
      "includes/manage_invoice.php?action=delete&invoice_number=" +
        invoice_number,
      true
    );
    xhttp.send();
...
function refresh() {
  ...
  xhttp.open("GET", "includes/manage_invoice.php?action=refresh", true);
  xhttp.send();
}
...
function searchInvoice(text, tag) {
  ...
  xhttp.open(
    "GET",
    "includes/manage_invoice.php?action=search&text=" + text + "&tag=" + tag,
    true
  );
  xhttp.send();
}
...
function printInvoice(invoice_number) {
  ...
  xhttp.open(
    "GET",
    "includes/manage_invoice.php?action=print_invoice&invoice_number=" +
      invoice_number,
    false
  );
  xhttp.send();
